...
public class TeacherValidator<T> {
    ...
    public static <T extends BaseDTO> Map<String, String> validate(T dto) {
        ...
        if (dto.getFatherName().length() < 2 | dto.getLastname().length() > 32) {
            errors.put("fatherName", "Father's Name must be between 2 and 32 characters");
        }

        if (dto.getEmail().contains("@") | dto.getEmail().length() < 2 | dto.getLastname().length() > 32) {
            errors.put("email", "Email must contain '@' and be between 2 and 32 characters");
        }
